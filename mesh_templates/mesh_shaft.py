...
def run(BODY_NAME, config):
    ...
    temp_dir = config.get('__temp_dir__', os.path.join(_ROOT, "_cache"))
    os.makedirs(temp_dir, exist_ok=True)
    AREA_CSV = f"{temp_dir}/shaft_area.csv".replace('\\', '/')
    ...
    apply_isoline_cyl(BODY_NAME, _v("mc.SHAFT_IsoLine_InnerRace.r_min", 7.4), _v("mc.SHAFT_IsoLine_InnerRace.r_max", 7.6), "SHAFT_IsoLine_InnerRace", _v("mc.SHAFT_IsoLine_InnerRace.axial_size", 2.5), _v("mc.SHAFT_IsoLine_InnerRace.cir_elems", 16), "0,255,255,")

    # MC2: Stator seating r=7.7-7.75mm, 2mm axial, 6 circ
    print("\n[MC2] Stator seating (r=7.7-7.75mm)...")
    apply_isoline_cyl(BODY_NAME, _v("mc.SHAFT_IsoLine_Stator.r_min", 7.7), _v("mc.SHAFT_IsoLine_Stator.r_max", 7.75), "SHAFT_IsoLine_Stator", _v("mc.SHAFT_IsoLine_Stator.axial_size", 2), _v("mc.SHAFT_IsoLine_Stator.cir_elems", 6), "0,255,0,")
    ...
    if not stator_cyl_faces:
        print("  X No stator seating faces — skipping ChangeLayers")
    else:
      ...
      <RedoFlag Value=""/><tag Value="-1"/>
        ...
        print(f"  Total mesh faces from group: {len(all_mesh_faces)}")
        # Fallback if group is empty — use getGeomEntitiesFromBody
        if not all_mesh_faces:
            print("  Group empty, using getGeomEntitiesFromBody fallback...")
            all_mesh_faces = simlab.getGeomEntitiesFromBody([BODY_NAME], mesh_model, 2)
            print(f"  Total mesh faces from fallback: {len(all_mesh_faces)}")
