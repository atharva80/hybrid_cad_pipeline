...
def generate_script(mesh_type_str):
    return f'''from hwx import simlab
...
def run(BODY_NAME, config):
  ...
  <SetCurrentDisplaySystem Name="MMKS (mm kg N C s)"/>
    ...
    mesh_type = _v('global.mesh_type', '{mesh_type_str}')
...
files_to_update = {
    "mesh_EPS.py": "Tet4",
    "mesh_boxes.py": "Hex",
    "mesh_canopies.py": "Tet10",
    "mesh_false_cover.py": "Tet4"
}

for filename, default_mesh in files_to_update.items():
    with open(os.path.join(base_dir, filename), "w") as f:
        f.write(generate_script(default_mesh))

print("All 4 templates successfully reverted to the robust SurfaceMesh + QCheck + VolumeMesh flow!")
