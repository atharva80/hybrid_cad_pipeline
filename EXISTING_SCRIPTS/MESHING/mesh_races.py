...
outer_vm = get_bodies_from_mesh(mesh_model, "OUTER_RACE")
outer_vm_str = "".join(f'"{b}",' for b in outer_vm)
print(f"  Volume mesh bodies: {outer_vm}")
volume_mesh(mesh_model, outer_vm_str, "OUTER_RACE", mesh_size, mesh_type)
print("  OK Volume mesh done")

print("\n  -- Merge -> OUTER_RACE --")
outer_actual = get_bodies_from_mesh(mesh_model, "OUTER_RACE")
print(f"  Bodies: {outer_actual}")
if len(outer_actual) >= 2:
    merge_bodies(outer_actual, "OUTER_RACE", mesh_model)
...
print(f"\n  -- Volume Mesh ({mesh_type}) --")
mid_vm = get_bodies_from_mesh(mesh_model, "MID_RACE")
mid_vm_str = "".join(f'"{b}",' for b in mid_vm)
print(f"  Volume mesh bodies: {mid_vm}")
volume_mesh(mesh_model, mid_vm_str, "MID_RACE", mesh_size, mesh_type)
print("  OK Volume mesh done")

print("\n  -- Merge -> MID_RACE --")
mid_actual = get_bodies_from_mesh(mesh_model, "MID_RACE")
print(f"  Bodies: {mid_actual}")
if len(mid_actual) >= 2:
    merge_bodies(mid_actual, "MID_RACE", mesh_model)
...
print(f"\n  -- Volume Mesh ({mesh_type}) --")
inner_vm = get_bodies_from_mesh(mesh_model, "INNER_RACE")
inner_vm_str = "".join(f'"{b}",' for b in inner_vm)
print(f"  Volume mesh bodies: {inner_vm}")
volume_mesh(mesh_model, inner_vm_str, "INNER_RACE", mesh_size, mesh_type)
print("  OK Volume mesh done")

print("\n  -- Merge -> INNER_RACE --")
inner_actual = get_bodies_from_mesh(mesh_model, "INNER_RACE")
print(f"  Bodies: {inner_actual}")
if len(inner_actual) >= 2:
    merge_bodies(inner_actual, "INNER_RACE", mesh_model)
